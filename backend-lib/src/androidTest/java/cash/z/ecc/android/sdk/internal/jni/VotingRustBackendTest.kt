...
class VotingRustBackendTest {
    ...
    fun voting_db_round_state_round_trips() =
        runTest {
            ...
            try {
                ...
                // Non-empty getVotesJson coverage belongs with the vote-insertion JNI wrapper.
    ...
    fun list_rounds_returns_all_rounds_for_current_wallet_only() =
        runTest {
            ...
            try {
                ...
                val firstWalletRounds =
                    JSONArray(firstWallet.listRoundsJson())
                        .toList()
                        .map { (it as JSONObject).getString("round_id") }
                        .toSet()
                val secondWalletRounds = JSONArray(secondWallet.listRoundsJson())

                assertEquals(setOf(ROUND_ID, SECOND_ROUND_ID), firstWalletRounds)
                assertEquals(0, secondWalletRounds.length())
            } finally {
                firstWallet.close()
                secondWallet.close()
            }
    ...
    fun generate_hotkey_is_deterministic_and_rejects_short_seed() =
        runTest {
            ...
            try {
                ...
                assertEquals(
                    FfiRoundPhase.HOTKEY_GENERATED,
    ...
    fun build_governance_pczt_returns_parseable_pczt_and_extractable_sighash() =
        runTest {
            ...
            try {
                ...
                assertEquals(
                    FfiRoundPhase.DELEGATION_CONSTRUCTED,
    ...
    private fun JSONArray.toList(): List<Any> =
        (0 until length()).map { index -> get(index) }
